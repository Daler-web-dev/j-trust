
export const uz = {
    header: {
        nav: {
            home: "Asosiy",
            about: {
                title: "Kompaniya Haqida",
                p1: "Litsenziyalar va sertifikatlar",
                p2: "Auditor sertifikatlari",
                p3: "Bo'sh ish o'rinlari",
                p4: "Mijozlar",
                p5: "Asoschilar",
                p6: "Ishlash prinsiplari",
                p7: "Bizning mutaxassislar",
                p8: "Boshqaruv"
            },
            services: {
                title: "Xizmatlar",
                p1: "Umumiy ma'lumot",
                p2: "Auditorlik texnologiyasi",
                p3: "Ko'rsatiladigan xizmatlar metodologiyasi",
                p4: "Majburiy audit",
                p5: "Initsiatil audit",
                p6: "Loyihalar va grantlar audit",
                p7: "Konsalting",
                p8: "Buxgalteriya xizmatlari"
            },
            auditReports: "Audit Hisobotlari",
            becameAClient: {
                title: "Mijoz bo'lish",
                p1: "Yordam",
            },
            links: {
                number_1: "+99890 919 36 90",
                number_2: "+99888 780 77 88",
                button_text: "Aloqa"
            }
        }
    },
    footer: {
        address: {
            title: "Javlon Audit Servis",
            street: "1) Samarqand shahri, Rudakiy ko‘chasi 140/18",
            street2: "2) Toshkent shahri, Uchtepa tumani, Qurilish, 3-tor ko‘chasi, Uy 3a, Kvartira 2"
        },
        about: {
            title: "Kompaniya Haqida",
            p1: "Litsenziyalar va sertifikatlar",
            p2: "Auditorlar sertifikatlari",
            p3: "Bo'sh ish o'rinlari",
            p4: "Mijozlar",
            p5: "Asoschilar",
            p6: "Ishlash prinsiplari",
            p7: "Bizning mutaxassislar",
            p8: "Boshqaruv"
        },
        services: {
            title: "Xizmatlar",
            p1: "Umumiy ma'lumot",
            p2: "Auditorlik texnologiyasi",
            p3: "Ko'rsatiladigan xizmatlar metodologiyasi",
            p4: "Majburiy audit",
            p5: "Initsiatil audit",
            p6: "Loyihalar va grantlar audit",
            p7: "Konsalting",
            p8: "Buxgalteriya xizmatlari"
        },
        more: {
            title: "Boshqa",
            auditReports: "Audit Hisobotlari",
            becameAClient: {
                title: "Mijoz bo'lish",
                p1: "Yordam",
            },
        },
        contacts: {
            title: "Aloqa",
            email: "javlon_audit@mail.ru",
            number_1: "+99891 523 57 75",
            number_2: "+99833 288 01 02",
        },
        company: {
            title: "Javlon Audit Servis",
            termsAndConditions: "Shartlar va shartlar",
            privacyPolicy: "Maxfiylik siyosati",
        }
    },
    "hero": {
        "company": "JAVLON AUDIT SERVIS",
        "title": "Audit tashkiloti",
        "subtitle": "buxgalteriya va soliq sohasida",
        "text": "«JAVLON AUDIT SERVIS» mas'uliyati cheklangan jamiyati sifatida faoliyat yurituvchi auditorlik tashkiloti Sizga auditorlik sohasida xizmatlarini taklif etishdan mamnun, Auditorlik faoliyati to'g'risidagi Qonun, Xalqaro auditorlik standartlari va buxgalteriya hamda moliyaviy hisobot standartlariga muvofiq.",
        "button": "Batafsil"
    },
    "financial": {
        "title": "Xizmatlar",
        "subtitle": "biz taklif qilamiz",
        "generalInformation": {
            "title": "Umumiy ma'lumot",
            "description": "XMH (Evropa hisobot standartlari), GAAP (AQSH hisobot standartlari) va boshqa mamlakatlar bo‘yicha moliyaviy hisobotlarni tayyorlash va konsolidatsiyalash."
        },
        "antiCrisisManagement": {
            "title": "Krizisga qarshi boshqaruv",
            "description": "U kompleks va tizimli xarakterga ega bo‘lib, biznes uchun noqulay hodisalarning oldini olish yoki bartaraf etishga qaratilgan bo‘lib, zamonaviy boshqaruvning barcha imkoniyatlaridan foydalaniladi."
        },
        "dueDiligence": {
            "title": "Due Diligence protsedurasi",
            "description": "Investitsiya obyekti to‘g‘risida ob'ektiv tasavvur hosil qilish jarayoni. Ushbu protseduralarni ob'ektiv va malakali o‘tkazishda ikki tomon ham manfaatdor."
        },
        "mandatoryAudit": {
            "title": "Majburiy audit",
            "description": "Moliyaviy hisobot va unga bog'liq moliyaviy ma'lumotlarning ishonchliligi va hisobotlarning muvofiqligini aniqlash uchun audit tashkilotlari tomonidan tekshiriladi."
        },
        "initiativeAudit": {
            "title": "Tashabbuskor audit",
            "description": "Bu xizmat korxona egalari uchun mo‘ljallangan bo‘lib, buxgalteriya hisobi holatini tekshirish, soliq hisobi holatini o‘rganish va korxonani boshqarish samaradorligini baholashga qaratilgan."
        },
        "auditOfProjects": {
            "title": "Loyihalar va grantlar auditi",
            "description": "O‘zbekiston Respublikasi byudjet mablag‘lari, Jahon banki, Osiyo taraqqiyot banki (OTB), BMTTD va boshqa notijorat tashkilotlari tomonidan moliyalashtirilgan loyihalar va grantlarning auditi o‘ziga xos xususiyatlarga ega."
        }
    },
    "servicesWeOffer": {
        "title": "XMH va ISA bo‘yicha audit xizmatlarini taklif qilamiz",
        "text_1": "Bugungi kunda Xalqaro moliyaviy hisobot standartlari (XMH) O‘zbekistonda tobora ommalashib bormoqda. 2015 yil 24 aprel kuni qabul qilingan Prezidentning PQ-4720-sonli farmoniga ko‘ra, 2015-2018 yillarda barcha aksiyadorlik jamiyatlari Xalqaro audit standartlari va Xalqaro moliyaviy hisobot standartlariga muvofiq yillik moliyaviy hisobotlarni e’lon qilish va tashqi auditni amalga oshirishga o‘tadilar. XMH va ISA bo‘yicha xizmatlar, shuningdek, O‘zbekiston hududida ro‘yxatdan o‘tgan korxonalarda ulushga ega bo‘lgan xorijiy investorlar uchun ham dolzarbdir.",
        "text_2": "XMH standartlari (XMH, GAAP) dunyoning ko‘plab kompaniyalarining moliyaviy hisobotlarini tayyorlashda qo‘llaniladi. O‘zbekiston kompaniyalari uchun XMH qo‘llanilishi menejerlarga boshqaruv samaradorligini sezilarli darajada oshiradigan ma'lumotlarni taqdim etadi va bozor va aksiyadorlar bilan malakali muloqot qilish imkonini beradi. Bizning XMH bo‘yicha xizmatlarimiz quyidagilarni o‘z ichiga oladi:",
        "button": "Batafsil"
    },
    "forma": {
        "title": "Maslahat so‘rovi",
        "name": "Ism",
        "surname": "Familiya",
        "email": "Elektron pochta",
        "phone": "Telefon raqami (O‘zbekiston)",
        "phone_2": "Telefon raqami majburiy",
        "phone_3": "Iltimos, haqiqiy O‘zbekiston telefon raqamini kiriting",
        "phone_4": "Iltimos, +998 bilan boshlanadigan haqiqiy O‘zbekiston telefon raqamini kiriting.",
        "button": "So‘rovni yuborish",
        "button_2": "Yuborilmoqda...",
        "toast": {
            "title": "Maslahat so‘rovi yuborildi",
            "description": "Biz tez orada siz bilan bog'lanamiz!"
        },
        error_toast: {
            title: "Nimadir noto'g'ri ketdi",
            description: "Ba'zi xatolik yuz berdi. Qayta urinib ko'ring!"
        }
    },
    aboutPage: {
        title: "Kompaniya Haqida",
        description_1: "«JAVLON AUDIT SERVIS» auditorlik tashkiloti audit xizmatlari bozorida o‘n sakkiz yildan ortiq faoliyat yuritib kelmoqda. Bizning tashkilot 2004 yil 1 martda tashkil etilgan. Tashkilotning ustav kapitali 240 000 000 so‘mni tashkil etadi. Yillik sug‘urta polisi 400 000 000 so‘m. Tashkilot mutaxassislari uch yildan yigirma yilgacha auditorlik tashkilotlarida ishlash tajribasiga ega va yirik sanoat korxonalari, tashkilotlar, aksiyadorlik jamiyatlari, mas’uliyati cheklangan jamiyatlar va xususiy korxonalar auditini o‘tkazishga ixtisoslashgan. Tashkilot milliy va xalqaro standartlarga muvofiq buxgalteriya hisobi tizimining metodologiyasiga ega bo‘lgan malakali mutaxassislarga ega bo‘lib, ular audit xizmatlarini ko‘rsatish va audit standartlariga muvofiq moliyaviy hisobotlarni tahlil qilish imkoniyatiga ega. Biz mijozlarimizga quyidagi xizmatlarni taklif etamiz: buxgalteriya hisobi standartlariga muvofiq audit standartlari bo‘yicha audit, buxgalteriya hisobi sohasidagi xizmatlar, buxgalteriya hisobi bo‘yicha tiklash va yuritish xizmatlari, soliq solish xizmatlari, konsalting, qurilish ishlari bo‘yicha ekspert xizmatlari, dasturiy ta’minot. Bizning asosiy missiyamiz mijozlarimizga ularning faoliyatini yaxshilashga yordam beradigan va ularning daromadlarini oshirishga yordam beradigan sifatli xizmatlar ko‘rsatish hisoblanadi.",

        description_2: "Auditorlik tashkilotlarining vazifalariga muvofiq biz xo‘jalik yurituvchi subyektning moliyaviy hisobotlarining ishonchliligini tahlil qilamiz va tasdiqlaymiz, xo‘jalik yurituvchi subyektlarning faoliyat samaradorligini oshirish bo‘yicha konstruktiv takliflar ishlab chiqamiz. Har bir mijozning xavf-xatarlarini va muammolarini aniqlagan holda, ularning individual ehtiyojlarini hisobga olib, mijozning qonunchilik, faoliyat va moliyaviy hisobot tizimi bilan bog‘liq barcha masalalarni to‘g‘ri aks ettirishga harakat qilamiz. Bizning auditorlik tashkilotimizning obro‘si biz uchun katta ahamiyatga ega va shu sababli auditorlik malaka sertifikatlariga ega bo‘lgan tashkilot mutaxassislari tashkilotning ijobiy imidjini yaratadi va qo‘llab-quvvatlaydi. Tashkilotda o‘n nafar xodim ishlaydi, jumladan: 4 nafar sertifikatlangan auditor, 4 nafar auditor yordamchilari, 1 nafar qimmatli qog‘ozlar bo‘yicha ekspert, 1 nafar qurilish mutaxassisi va boshqalar shartnoma asosida. Ikki nafar mutaxassis CIPA sertifikatlariga (Sertifikatlangan xalqaro professional buxgalter) ega, besh nafar mutaxassis esa CAP (Sertifikatlangan buxgalter amaliyotchisi) sertifikatlariga ega. Buyurtmachining talabiga binoan biz uning manfaatlarini soliq va boshqa organlar tomonidan o‘tkaziladigan tekshiruvlarda himoya qilamiz, tekshiruv natijalari bo‘yicha to‘liq javobgarlikni o‘z zimmamizga olamiz va auditdan so‘ng bir yil davomida bepul maslahatlar beramiz. Biz audit xizmatlarining yuqori sifatini kafolatlaymiz va noto‘g‘ri auditorlik xulosasi uchun javobgarlikni o‘z zimmamizga olamiz. Diqqat uchun samimiy minnatdorchilik bildiramiz va uzoq muddatli o‘zaro manfaatli hamkorlikka umid qilamiz. Agar siz bizning tashkilotimiz bilan hamkorlik qilishga qiziqsangiz, istalgan vaqtda uchrashishga tayyormiz va muhokama qilishga tayyormiz.",

        description_3: "Audit narxi ish hajmi va audit o‘tkazish uchun zarur bo‘lgan mehnat xarajatlari, shuningdek, audit o‘tkaziladigan davr, filiallar mavjudligi, pul oqimlari hajmi, korxona hujjat aylanishi, ishchilar soni, faoliyat turi va korxonaning qaysi soliqlarni to‘lovchi ekanligiga asoslanadi.",
        services: {
            title: "MCHJ 'Javlon Audit Servis' tomonidan taqdim etilayotgan xizmatlar:",
            opt: [
                "Moliyaviy hisobotlarni audit qilish",
                "Moliyaviy hisobotlarni IFRS ga muvofiq aylantirish",
                "Iqtisodiy faoliyat, bojxona qonunchiligi, hisob va soliq bo'yicha maslahatlar",
                "Korxonalar uchun soliqni optimallashtirish",
                "Korxona faoliyatini moliyaviy tahlil qilish",
                "Ustav kapitalini shakllantirishning to'g'riligini audit qilish",
                "Hisobni tiklash va yuritish",
                "Maxsus masala audit",
                "Korporativ boshqaruv tizimini baholash",
                "Xarid va tender jarayonlarini audit qilish",
                "Hisob siyosatini ishlab chiqish (NASB va IFRS)",
                "Soliq siyosatini ishlab chiqish",
                "Hujjat aylanishi bo'yicha nizomlarni ishlab chiqish",
                "Qimmatli qog'ozlar va investitsiya paketlarini audit qilish",
                "Due diligence (olingan kompaniya audit)",
                "Mulkni inventarizatsiya qilish",
                "Boshqa xizmatlar"
            ]
        },
        clients: {
            title: "Kompaniyaning Mijozlari:",
            companies: [
                {
                    title: "Temir Yo'llar",
                    description: "GAJK 'O’zbekiston Temir Yullari' (98 bo'linma), MCHJ 'O’ztemiryolmashta’mir', Temir ulash korxonasi № 14, SSMT 406, AJ 'YUKTRANS', DP Qozon va Mexanik zavod, Tashkent viloyati Temir yo'l markazi, Buxoro viloyati Temir yo'l markazi, Termiz viloyati Temir yo'l markazi, Qo'qon viloyati Temir yo'l markazi, Qarshi viloyati Temir yo'l markazi, Kungrad viloyati Temir yo'l markazi."
                },
                {
                    title: "Neft, Gaz va Ko'mir Qidirish Korxonalari",
                    description: "AJ 'Uztransgaz', DP 'Tashgaztaminot', DP 'Tashshahar gaz', AJ 'Andijanneft', AJ 'Uzgeoburneftgaz', AJ 'Sharg Unkomir', DP 'Korovulbozor Texnikasi', UDP 'Muborak Texnikasi', MCHJ 'AGTKSH – Nukus', JV MCHJ 'Uz-Korgaschemical'."
                },
                {
                    title: "Energiya Korxonalari",
                    description: "AJ 'Navoi TPP', AJ 'Farg'ona TPP', AJ 'Buxoro TPP', AJ 'Sirdaryo TPP', AJ 'Chirchiq GESlari kaskadi', AJ 'Elektr Tarborkurilish', UP PO 'Tashtetloenergo', AJ 'Tashelektroshchit', MCHJ 'Elektroizolit', JV AJ 'UzKabel', AJ 'Tashkommunaltaminot', JV AJ 'Uzelektroapparat-electroshchit', JV 'RFN-Kompressor Filter', JV AJ 'Chirchiq Transformer zavodi'."
                },
                {
                    title: "Qurilish Korxonalari, Qurilish Materiallari Ishlab Chiqarish, Loyihalash Institutlari",
                    description: "OAO SRP 'Energotamrir', OAO 'Gidrospetsstroy', OAO 'RSMU-4', AOOOT 'Uzogirsanotloyikha', JV 'StroyIndustriya', AJ 'Tashstroireyremontdor Trust', OAO 'Uzkimyosanotloyikha', ZAO 'Uzlitineftegaz', JV MCHJ 'CO PASCHAL FORMS LTD', JV MCHJ 'SILKCOATPAINT', IP MCHJ 'NORDLIKALUMINUMBUILDINGMATERIALS', JV 'Tezintom' (metal tile ishlab chiqarish)."
                },
                {
                    title: "Farmatsevtika Ishlab Chiqarish va Farmatsevtika Korxonalari",
                    description: "JV 'Jurabeklaboratories LTD', OAO 'Uzkhimfarm', DP 'MED GLASS', JV 'Universal Pharm', JV 'FARM GLASS', JV 'TashTamedPharma' (ishlab chiqarish), IP MCHJ 'PharmaciaUniversal', MCHJ 'Temireyolfarmatsya', MCHJ 'As-Farma', OAO RTPF 'Tijorat', MCHJ 'Yunimed', OAO 'DoriDarmon', MCHJ 'FARMEXX', IP MCHJ 'VELFARMSTARTRADE', IP MCHJ 'QURAMAXMEDIKAL', MCHJ 'OROM-BIOPREPARAT', MCHJ 'OSTEOSINTEZ-CHM', MCHJ 'MONADA – AMON'."
                },
                {
                    title: "Neft va Yog', Paxta va Un Sanoatlari",
                    description: "AK 'Uzdonmahsulot', AOOOT 'Kokand Yog'moy', AOOOT 'Surkhandarya Zikovkat Sanoyat', OAO 'Buxoro Ekstrakt Yog', OAO 'Koson Yog - Ekstraktsiya', OAO 'Kukon Don Makhslulot', OAO 'DEMET', JV MCHJ 'GOOD FOOD GROUP', MCHJ 'Tashkent-Tola', JV 'OltinChigit', PTC 'Buxoro Trans Terminal', MCHJ 'Uzagrodon', MCHJ 'Parwaz Pakhtatermini', MCHJ 'Mahsus Agroservis', OAO 'Agrotexminot-BS'."
                },
                {
                    title: "Tashqi Iqtisodiy Faoliyat, Birjalar",
                    description: "MCHJ 'Uzvneshtrans', MCHJ 'Respublikalararo Universall Agrar-Moliya Birjasi', GAVK 'Uzmarkazimpeks', Jahon Iqtisodiyoti va Diplomatiyasi Universiteti Tashkiloti."
                },
                {
                    title: "Kimyo Sanoati",
                    description: "JV 'MAXAMUZBEKISTAN', OAO 'MAXAMAMMOFOS', MCHJ 'ASTRACHEM-TASHKENT', MCHJ 'Galennika', JV 'BAWCHEMICALLTD'."
                },
                {
                    title: "Avtomobil Sanoati va Transport Korxonalari",
                    description: "ZAO 'General Motors Powertrain – Uzbekistan', JV 'KAMAZ' Avto Markazi, AK 'Uzavtosanoat', OAO 'Tashviloyatavtotexhizmat', MCHJ 'AVTOSANKOMPLEKT', Avtobus Deposi Tashmetrostroy, MCHJ 'Eximtransservis', MCHJ 'KIMYOTRANS', DP 'UZAVTOMOTOBELGI'."
                },
                {
                    title: "Nashriyot Uylar va Bosmaxonalar",
                    description: "ID 'OsiyoTayms' (Darakchi gazetasi), ID 'Birja', MCHJ 'Poytaht-Inform', Gazeta 'O'zbekiston O'qituvchisi' (M'arifat), GP 'Toshkent viloyati Gazetasi', Jamoat Fondi 'Ijod' O'zbekiston Yozuvchilar Uyushmasi tasarrufida, MCHJ 'PRESSTIJPRINT', O'zbekiston Mudofaa Vazirligi Cartografik Markazi."
                },
                {
                    title: "Sinov va Sertifikatlash Markazlari va Institutlari",
                    description: "GP 'Respublika Sinov va Sertifikatlash Markazi', GP Buxoro CIS, GP 'Toshkent Shahar Sinov va Sertifikatlash Markazi', Standartlashtirish, Metrologiya va Sertifikatlashtirish Institutining Tadqiqot Markazi, MCHJ 'Russian Register – Markaziy Osiyo'."
                },
                {
                    title: "Lizing va Sug'urta Korxonalari",
                    description: "ULK 'Baraka', JV 'Alskom' (sug'urta kompaniyasi), SK 'ArkSugurta', SK 'FinansSugurta', IP MCHJ 'Business Leasing', MCHJ 'Orient Leasing', OAO 'Uzaviat Leasing', AK 'Avi Leasing' (aviatsiya xizmati), LK 'Asaka Trans Leasing', ZAO 'Pahta Leasing', OAO 'UzMED-Leasing', OAO 'Kurilish-Leasing', MCHJ 'Uzavtosanoat – Leasing', va boshqalar."
                },
                {
                    title: "Sport va Salomatlik Tashkilotlari va Uyushmalari",
                    description: "O'zbekiston Bokschilar Uyushmasi, O'zbekiston Golf Uyushmasi, Markaziy Sport va Sog'liqni Saqlash Klubi 'Lokomotiv', JV 'ILDS' (golf klubi), MCHJ 'BANKLARAROSPORTVASOGLANMASHTIRISHMARKAZI', GP 'Oromgoh Kompaniyasi', OAO 'Chatkal', GP 'Sanatoriya Botanika'."
                },
                {
                    title: "O'zbekiston Respublikasi/Karakalpakstandagi Loyihalar/Grantlar:",
                    description: "JFPR/ADB №35447-01 'O'zbekistondagi Qishloq Joylarda Suv Ta'minoti va Sanitariya Holatini Yaxshilash', O'zbekiston Moliya va Mehnat Munosabatlari Vazirligi Dasturi, 'Qishloq Joylarda Mikro Moliyalash' Loyihasi 'Fermer', 'Altyn Asir' Vazirlar Mahkamasining Dasturi, Xalqaro Mehnat Tashkilotining 'Qashshoq uchun Ish O'rinlari Ta'minlash' Dasturi, Dunyo Bankining Mehnat Bozori Rivojlantirish Dasturi va boshqalar."
                }
            ]
        },
    },
    "aboutCompanyPage": {
        certificates: {
            title: "Sertifikatlar",
        },
        auditorCertificates: {
            title: "Auditorlar sertifikatlari",
        },
        vacancies: {
            jobListing: {
                careerTitle: "Kompaniyada karera",
                careerDescription: "Hozirda audit tashkiloti MChJ 'JAVLON AUDIT SERVIS' ishga taklif qiladi:",
                auditors: "Sertifikatlangan audito'rlar",
                locationHeader: "Manzil",
                positionHeader: "Bo'sh lavozimlar",
                positions: [
                    { location: "Toshkent", position: "Birlamchi toifali audito'rlar" },
                    { location: "Samarqand", position: "Samarqand filiali rahbari" },
                    { location: "Nukus", position: "Nukus filiali rahbari" },
                    { location: "Xorazm", position: "Xorazm filiali rahbari" },
                    { location: "Navoi", position: "Navoi filiali rahbari" },
                ],
                requirements: "Malaka talablari:",
                requirementsList: [
                    "Moliyaviy vazirlikdan auditorlik sertifikatiga ega bo'lish,",
                    "Oliy iqtisodiy yoki moliyaviy ta'lim,",
                    "Bosh buxgalter yoki auditor sifatida kamida 5 yil ish staji,",
                    "CAP sertifikatiga ega bo'lish xush keladi."
                ],
                registrationNotice: "Ish joyiga mos ro'yxatdan o'tish.",
            },
            forma: {
                formTitle: "Barcha * belgilangan maydonlar to'ldirilishi shart.",
                personalInfo: "Shaxsiy ma'lumotlar",
                firstName: "Ism *",
                lastName: "Familiya *",
                email: "E-mail *",
                phone: "Telefon *",
                age: "Yosh *",
                position: "Taklif etilgan lavozim *",
                languages: "Til bilish darajasi",
                skills: "Kompyuter ko'nikmalari va bilimlari",
                additionalSkills: "Qo'shimcha ko'nikmalar",
                workExperience: "Ish tajribasi",
                startDate: "Boshlanish sanasi",
                endDate: "Tugash sanasi",
                company: "Kompaniya nomi",
                companyDescription: "Kompaniya faoliyati haqida qisqacha ma'lumot",
                jobTitle: "Sizning lavozimingiz",
                responsibilities: "Lavozim vazifalari",
                education: "Ta'lim",
                eduStartDate: "Boshlanish sanasi",
                eduEndDate: "Tugash sanasi",
                institution: "Ta'lim muassasasi nomi",
                degree: "Olingan daraja",
                specialization: "Diplomdagi mutaxassislik",
                certificates: "Sertifikatlar",
                certName: "Nomi",
                certDescription: "Qisqacha tavsifi",
                submit: "Yuborish",
                requiredField: "Bu maydon to'ldirilishi shart.",
                skillsPlaceholder: `Kompyuter ko'nikmalaringizni yozing (maks. 200 belgidan iborat)`,
                additionalSkillsPlaceholder: `Qo'shimcha ko'nikmalar (maks. 200 belgidan iborat)`,
                responsibilitiesPlaceholder: `Lavozim vazifalaringizni tasvirlang (maks. 200 belgidan iborat)`,
                certDescriptionPlaceholder: `Sertifikatni tasvirlang (maks. 200 belgidan iborat)`,
                "toast": {
                    "title": "So‘rov yuborildi",
                    "description": "Biz tez orada siz bilan bog'lanamiz!"
                },
                error_toast: {
                    title: "Nimadir noto'g'ri ketdi",
                    description: "Ba'zi xatolik yuz berdi. Qayta urinib ko'ring!"
                }
            }
        },
        "clients": {
            "title": "Mijozlarimiz",
            "description": "Hozirda 'JAVLON AUDIT SERVIS' auditorlik kompaniyasi turli xil va barcha mulk shakllaridagi tashkilotlar bilan hamkorlik qilishda davom etmoqda va ularga auditorlik xizmatlarini ko'rsatmoqda. Mijozlarimiz quyidagilarni o'z ichiga oladi:",
            "nonProfit": {
                "title": "Notijorat tashkilotlar",
                "companies": [
                    "O'zbekiston Respublikasi Banklar Assotsiatsiyasi",
                    "Ijtimoiy tashabbuslarni qo'llab-quvvatlash jamg'armasi (FOPSI)",
                    "Moliya vazirligining axborot-hisoblash markazi",
                    "'XEROX' kompaniyasining vakolatxonasi",
                    "NBU VIP xizmat",
                    "Aral dengizi genofondini himoya qilish xayriya jamg'armasi",
                    "O'zbekistonda 'World Vision' xalqaro xayriya tashkilotining vakolatxonasi",
                    "Fan va innovatsiyalar markazi",
                    "O'zbekiston Respublikasi Bank-moliya akademiyasi",
                    "O'zR Fanlar akademiyasi Polimerlar kimyosi va fizikasi instituti",
                    "Standartlashtirish, metrologiya va sertifikatlashtirish ilmiy-tadqiqot instituti",
                    "NAESMI jamg'armasi",
                    "Milliy ommaviy axborot vositalari assotsiatsiyasi",
                    "UzRIT",
                    "Oliy ta'lim vazirligining CSSPO",
                    "O'zbekiston Statistika boshqarmasi",
                    "Kredit uyushmalari assotsiatsiyasi",
                    "INTILLISH jamg'armasi",
                    "Uy-joy xususiylashtirish bo'yicha shahar markazi",
                    "Multimedia texnologiyalarini rivojlantirish markazi",
                    "Chiqish oldi moslashuv markazi va boshqalar."
                ]
            },
            "industrial": {
                "title": "Sanoat kompaniyalari",
                "companies": [
                    "O'zbekiston temir yo'llari (98 filial)",
                    "Toshkentgaz ta’minot (31 filial) - 'Uztransgaz' bo'limi",
                    "Respublika Inkasso Uyushmasi (13 filial)",
                    "'Andijonneft' aksiyadorlik jamiyati",
                    "Sirdaryo issiqlik elektr stansiyasi",
                    "Navoiy issiqlik elektr stansiyasi",
                    "Farg'ona elektr tarmoqlari",
                    "RSOBK 'Attorlik'",
                    "'Uzlitineftegaz' aksiyadorlik jamiyati",
                    "Buxoro elektr tarmoqlari",
                    "Chirchiq gidroelektr stansiyalari kaskadi",
                    "'Elektrotarmokkurilish' aksiyadorlik jamiyati",
                    "Toshtepeloenergo",
                    "'Tashelektroshchit' aksiyadorlik jamiyati",
                    "Elektroizolit",
                    "Energoimpex",
                    "SRP 'Energota'mir'",
                    "Gazlineftegazdobycha",
                    "'O'ztemiryolmashtamir' aksiyadorlik jamiyati",
                    "Relssvarka zavodi N 14",
                    "SSMP 406",
                    "'Kokand yog'-moy' zavodi",
                    "Surxon oziq-ovqat sanoati",
                    "'Algoritm' aksiyadorlik jamiyati",
                    "Boshqalar."
                ]
            },
            "foreignInvestment": {
                "title": "Xorijiy investitsiyalarga ega kompaniyalar",
                "companies": [
                    "'UzKabel' qo'shma korxonasi",
                    "'MAXAM Uzbekistan' qo'shma korxonasi",
                    "'MAXAM Ammofos' aksiyadorlik jamiyati",
                    "'Sanoatqalinqogozsavdo' aksiyadorlik jamiyati",
                    "'Tezintom' qo'shma korxonasi (metall cherepitsa)",
                    "'M&M Militzer&Muench' davlat korxonasi (pochta)",
                    "'Emex-Tashkent' davlat korxonasi (pochta)",
                    "'Uzmakom' qo'shma korxonasi (aloqa)",
                    "'ITPS – Asia' qo'shma korxonasi (aloqa)",
                    "'Kasansay-Tekmen' qo'shma korxonasi (to'qimachilik)",
                    "'MED GLASS' davlat korxonasi (stakan ishlab chiqarish)",
                    "'Universal Farm' qo'shma korxonasi (farmatsevtika)",
                    "'FARM GLASS' qo'shma korxonasi (farmatsevtika)",
                    "'Oltin Chigit' qo'shma korxonasi",
                    "'MPS-Tashkent' davlat korxonasi (xizmatlar)",
                    "'Stroy Industriya' qo'shma korxonasi (qurilish)",
                    "'UNIWEAR' qo'shma korxonasi (tikuvchilik)",
                    "'RFN-Kompressor Filter' qo'shma korxonasi",
                    "'Uzelektroapparat' qo'shma korxonasi",
                    "'Jurabek Laboratories LTD' qo'shma korxonasi",
                    "KAMAZ Avtoservisi",
                    "'BETA Algoritm' qo'shma korxonasi",
                    "'Buxoro Trans Terminal' kompaniyasi",
                    "NPO 'FONON'",
                    "'Region Uz Oil' davlat korxonasi va boshqalar."
                ]
            },
            "unions": {
                "title": "Lizing, sug'urta kompaniyalari va kredit uyushmalari",
                "companies": [
                    "'Baraka' lizing kompaniyasi",
                    "'Poytaxt Lizing' xususiy korxonasi",
                    "'Minitech Invest Lizing' aksiyadorlik jamiyati",
                    "'Alskom' sug'urta kompaniyasi",
                    "'Ark Sugurta' sug'urta kompaniyasi",
                    "'Finans Sugurta' sug'urta kompaniyasi",
                    "'Biznes Lizing' xususiy korxonasi",
                    "'Orient Lizing' mas'uliyati cheklangan jamiyati",
                    "'Uzaviolizing' aksiyadorlik jamiyati",
                    "'Aviolizing' aviatsiya xizmati aksiyadorlik jamiyati",
                    "'Asaka Trans Lizing' lizing kompaniyasi",
                    "'Paxta Lizing' aksiyadorlik jamiyati",
                    "'Uzmed Lizing' aksiyadorlik jamiyati",
                    "'Kurlish Lizing' aksiyadorlik jamiyati",
                    "'Union Tash' kredit uyushmasi",
                    "'Osie Trust' kredit uyushmasi",
                    "'Turon' kredit uyushmasi",
                    "'Kapital Plus' mikrokredit uyushmasi",
                    "'Delta' kredit uyushmasi va boshqalar."
                ]
            },
            "trading": {
                "title": "Savdo kompaniyalari",
                "companies": [
                    "'Uzmarkazimpex' davlat aksiyadorlik kompaniyasi",
                    "'Uzelectrokabelsavdo' aksiyadorlik jamiyati",
                    "Respublika universal agro sanoat birjasi",
                    "'Rosella Trading' mas'uliyati cheklangan jamiyati",
                    "'Uzulgurjisavdo' aksiyadorlik jamiyati",
                    "'RISE Land Motors' qo'shma korxonasi",
                    "'Temireyolfarmatsia' mas'uliyati cheklangan jamiyati",
                    "'As-Farma' mas'uliyati cheklangan jamiyati",
                    "'Poytaxt-Inform' mas'uliyati cheklangan jamiyati",
                    "'Tijorat' aksiyadorlik jamiyati",
                    "'Sharki Kuroma' aksiyadorlik jamiyati",
                    "'Unimed' mas'uliyati cheklangan jamiyati",
                    "'Uzfarmsanoat' aksiyadorlik jamiyati",
                    "'MT Invest-Impex' respublika fondi",
                    "'Uzulgurjisavdo' aksiyadorlik jamiyati",
                    "'Oriental Management' davlat korxonasi",
                    "'Anglesey Food' davlat korxonasi",
                    "'Belton Trading Asia' davlat korxonasi va boshqalar."
                ]
            }
        },
        "founders": {
            "title": "Tashkilotchilar",
            "name": "MChJ 'Javlon Audit Servis' asoschisi Nuridinova Shaxnoza Tairovna",
            "description": "Asosiy faoliyat — mijozlarga auditorlik faoliyati to‘g‘risidagi qonunga, xalqaro auditorlik standartlariga va buxgalteriya hisobi standartlariga muvofiq auditorlik tekshiruvlari va ularga bog‘liq xizmatlarni, shuningdek, buxgalteriya hisobi sohasidagi xizmatlar, buxgalteriya hisobini tiklash va yuritish xizmatlari, soliq solish xizmatlari, konsalting va dasturiy ta'minot xizmatlarini ko‘rsatish."
        },
        specialists: {
            "ourSpecialists": [
                "Savollaringizga professional javob berishadi, O'zbekiston qonunchiligi va agar kerak bo'lsa, xalqaro qonunchilikka murojaat qilib, bu keyinchalik raqiblar bilan yuzaga keladigan nizolar paytida dalil sifatida ishlatilishi mumkin;",
                "Sizga zarur kafolatlarni taqdim etadilar, chunki berilgan tavsiyalar uchun firma har doim javobgarlikni o'z zimmasiga oladi."
            ],
            "qualificationRequirementsForSpecialists": {
                "mainRequirements": [
                    "Auditorlik xulosasini imzolash huquqiga ega bo'lgan barcha toifadagi auditorlarga qo'yiladigan malaka talablari O'zbekiston Respublikasi “Auditorlik faoliyati to'g'risidagi” Qonunga muvofiq belgilanadi, xususan: auditor sertifikatiga ega bo'lish, oliy iqtisodiy (moliyaviy yoki matematik) ma'lumotga ega bo'lish; bosh hisobchi lavozimida kamida besh yil ish stajiga ega bo'lish; normativ hujjatlar, Qonunlar, Ko'rsatmalar va korxonalar faoliyatini tartibga soluvchi boshqa me'yoriy hujjatlarni bilish."
                ],
                "requirementsForAuditorAssistants": [
                    "Oliy iqtisodiy/moliyaviy yoki matematik ma'lumot;",
                    "Bosh hisobchi lavozimida kamida uch yil ish stajiga ega bo'lish; normativ hujjatlar, Qonunlar, Ko'rsatmalar va boshqa me'yoriy hujjatlarni bilish."
                ]
            },
            "complianceWithAuditorEthics": {
                "description": "Bizning tashkilotda auditorlar va auditor yordamchilari O'zbekiston auditorlarining kasbiy odob-axloq kodeksiga rioya qiladilar."
            },
            sectionTitles: {
                ourSpecialists: "Bizning mutaxassislar",
                qualificationRequirements: "Kvalifikatsiya talablari",
                mainRequirements: "Asosiy talablar",
                requirementsForAuditorAssistants: "Auditor yordamchilari uchun talablar",
                complianceWithAuditorEthics: "Auditor etikasiga rioya qilish",
            },
        },
        management: {
            "title": "Rahbariyat",
            "clientPrinciple": "Mijozlar bilan ishlashdagi asosiy tamoyil — halollik, mas'uliyat va maxfiylik, chunki ishonch puldan ham ko‘proq va aylanma va barqarorlikdan ham muhimroqdir.",
            "managementPrinciple": "Auditorlik kompaniyasi rahbariyatining asosiy tamoyili — yuqori imidjni saqlash, byudjet bilan moliyaviy hisob-kitoblardagi halollik, jamoat faoliyatida ishtirok etish, xodimlarning munosib ish haqi va moddiy-texnik bazani ta'minlash hamda yuqori sifatli xizmatlarni taqdim etish."
        }
    },
    "workPrinciples": {
        "title": "Ish Prinsiplari",
        "card": {
            "title": "Auditor- Hamkorni Boshqarish",
            "description": "Bosh auditor audit xulosasini imzolash huquqiga ega va auditni o'tkazishda to'liq javobgarlikni o'z zimmasiga oladi. Bosh auditor auditorlarni va yordamchi auditorlarni (assistentlarni) ularning javobgarliklari va bajarishi kerak bo'lgan protseduralarning maqsadlari haqida o'rgatishi shart, auditi o'tkazilayotgan sub'ekt faoliyati va audit protseduralarining mohiyati, vaqti va ko'lami ustida ta'sir ko'rsatishi mumkin bo'lgan hisobga olish muammolari haqida. Bosh auditor audit jarayonida doimiy ravishda ishtirok etishi kerak...",
            "date": "2021-yil 24-yanvar"
        },
        "partnerAuditorControl": {
            "title": "Auditor-Hamkorni Boshqarish",
            "description": [
                "Auditor-hamkor audit xulosasini imzolash huquqiga ega va auditni o'tkazishda to'liq javobgarlikni o'z zimmasiga oladi.",
                "Auditor-hamkor auditorlarni va yordamchi auditorlarni (assistentlarni) ularning javobgarliklari va bajarilishi kerak bo'lgan protseduralarning maqsadlari haqida o'rgatishi shart, audit o'tkazilayotgan sub'ekt faoliyati va audit protseduralarining mohiyati, vaqti va ko'lami ustida ta'sir ko'rsatishi mumkin bo'lgan hisobga olish muammolari haqida.",
                "Auditor-hamkor audit jarayonida doimiy ravishda boshqarish va yo'naltirishni amalga oshirishi kerak, bu esa yordamchi auditorlar va yordamchi auditorlarning (assistentlarning) bajaradigan ishlarini nazorat qilishga qaratilgan.",
                "Auditor-hamkor va boshqaruvchi funksiyalarga ega auditorlar, ularga berilgan ishni bajaruvchi yordamchi auditorlarning professional kompetentsiyasini hisobga oladi, bu esa ularning ishiga bo'lgan yo'nalish va nazorat darajasini belgilashda hisobga olinadi.",
                "Yordamchi auditor tomonidan bajarilgan har qanday ishning o'z vaqtida bajarilishi, zaruriy professional kompetentsiyaga ega bo'lgan shaxslar tomonidan bajarilishiga kafolat berish uchun shunday amalga oshirilishi kerakki, bu professional kompetentsiya darajasi berilgan shartlarga muvofiq bo'lsin.",
                "Auditor-hamkor yordamchilarga audit dasturi va audit yo'nalishini tanishtiradi.",
                "Audit jarayonini amalda ko'rib chiqishda, ijro etuvchi boshqaruvchi quyidagi majburiyatlarga ega:",
                "1) Audit jarayonining dinamikasini kuzatishni amalga oshirish uchun:",
                "   - auditor yordamchisi o'z vazifalarini bajarish uchun zaruriy bilim va kompetentsiyaga ega ekanligini aniqlash;",
                "   - ish umumiy reja va audit dasturiga muvofiq amalga oshirilayotganligini aniqlash.",
                "2) Audit jarayonida paydo bo'lgan muhim masalalarni muhokama qilish va ko'rib chiqish, bu esa ularning ahamiyatini ko'rsatish va umumiy reja va audit dasturiga tegishli o'zgarishlarni kiritish orqali.",
                "3) Xodimlar o'rtasida nomuvofiqliklar va professional qarorlarni o'rnatish va maslahatchilik darajasini belgilash.",
                "Har bir auditor yordamchisining bajarishi lozim bo'lgan ish, kamida teng darajadagi kompetentsiyaga ega bo'lgan xodimlar tomonidan ko'rib chiqilishi lozim, bu esa quyidagilarni ko'rib chiqish uchun:",
                "   a) muvofiqlik uchun ishni tegishli vazifalar bilan;",
                "   b) barcha hujjatlar, ish vazifalari va olingan natijalar to'liq bajarilganligini aniqlash;",
                "   c) audit natijalarida barcha muhim masalalar aks etganligini aniqlash;",
                "   d) olingan natijalarga muvofiq ravishda chiqarilgan xulosalar audit fikrini tasdiqlash.",
                "Quyidagi masalalar o'z vaqtida ko'rib chiqilishi zarur:",
                "   a) umumiy audit rejasi va dasturi;",
                "   b) noo'rin risk va nazoratlarning mavjudligi, shu jumladan, nazorat sinovlarining natijalari va agar bunday bo'lsa, umumiy audit rejasida va dasturida o'zgarishlar kiritilganligi;",
                "   c) asosiy tekshirish protseduralarini bajarish natijasida olingan nazorat natijalari haqida xabardor qilish, va ularning asosida chiqarilgan xulosalar, shu jumladan, maslahatlarning natijalari;",
                "   d) ish hujjatlari, taklif qilingan audit o'zgartirishlari va taklif qilingan audit hisobotini ko'rib chiqish."
            ]
        }
    },
    services: {
        main: {
            "title": "Xizmatlar",
            "card_1": {
                "title": "Umumiy Ma'lumot",
                "description": "Moliya hisobotini tayyorlash va konsolidatsiya qilish, Xalqaro Moliya Hisobot Standartlari (IFRS), AQSh buxgalteriya standartlari (GAAP), va boshqa davlatlar standartlariga muvofiq."
            },
            "card_2": {
                "title": "Inqirozga qarshi boshqaruv",
                "description": "Bu zamonaviy boshqaruv vositalaridan foydalanish orqali biznes uchun salbiy holatlarni oldini olish yoki bartaraf etishga qaratilgan keng qamrovli, tizimli xarakterga ega."
            },
            "card_3": {
                "title": "Due Diligence Jarayoni",
                "description": "Investitsiya obyekti bo'yicha obyektiv fikr shakllantirish jarayoni. Ushbu jarayonlarni ob'ektiv va malakali o'tkazishda ikkala tomon ham manfaatdor."
            },
            "card_4": {
                "title": "Majburiy audit",
                "description": "Moliya hisobotini va tegishli moliyaviy ma'lumotlarni auditorlik tashkilotlari tomonidan tekshirish, moliyaviy hisobotlarning to'g'riligi va muvofiqligini aniqlash uchun."
            },
            "card_5": {
                "title": "Ixtiyoriy audit",
                "description": "Bu xizmat, hisob raqamlarini tekshirish, soliq hisobini ko'rib chiqish va biznesni boshqarish samaradorligini baholashga qaratilgan."
            },
            "card_6": {
                "title": "Loyihalar va grantlar bo'yicha audit",
                "description": "O'zbekiston Respublikasi byudjetidan, Jahon Bankidan, Osiyo Taraqqiyot Bankidan, BMT rivojlantirish dasturidan va boshqa NNTlardan moliyalashtiriladigan loyihalar va grantlar bo'yicha audit."
            },
            "card_7": {
                "title": "Konsalting",
                "description": "Agar siz buxgalteriya hisobini yuritish va soliqlarni hisoblashda xatolardan qochmoqchi bo'lsangiz, agar sizda savollar bo'lsa."
            },
            "card_8": {
                "title": "Buxgalteriya xizmatlari",
                "description": "Bu keng tarqalgan haqiqat bo'lishi mumkin, lekin zamonaviy sharoitlarda biznesni samarali boshqarish uchun o'z vaqtida va ishonchli ma'lumotlarga ega bo'lmaslik qiyin."
            }
        },
        "generalInformation": {
            "title": "Umumiy Ma'lumot",
            "text_1": "«JAVLON AUDIT SERVIS» auditorlik tashkiloti auditorlik xizmatlari bozorida o‘n sakkiz yildan ortiq vaqtdan beri faoliyat yuritmoqda. Bizning tashkilotimiz 2004 yil 1 martda tashkil etilgan. Tashkilotning ustav fondi 240 000 000 so‘mni tashkil etadi, yillik sug‘urta polisi esa 400 000 000 so‘mga teng. Tashkilotning mutaxassislari auditorlik tashkilotlarida uch yildan yigirma yilgacha bo‘lgan tajribaga ega bo‘lib, yirik sanoat korxonalari, tashkilotlar, aksiyadorlik jamiyatlari, mas'uliyati cheklangan jamiyatlar va xususiy korxonalarda tekshiruv o‘tkazishga ixtisoslashgan. Tashkilotda milliy va xalqaro standartlarga muvofiq buxgalteriya tizimi metodologiyasini yaxshi biladigan malakali mutaxassislar mavjud bo‘lib, ular audit xizmatlarini ko‘rsatishlari va audit standartlariga muvofiq moliyaviy hisobotlarni tahlil qilishlari mumkin. Biz o‘z mijozlarimizga quyidagi xizmatlarni taklif etamiz: audit, buxgalteriya hisobi standartlariga muvofiq buxgalteriya hisobi xizmatlari, buxgalteriya hisobotlarini tiklash va yuritish, soliq xizmatlari, konsalting, qurilish ishlari bo‘yicha ekspertlar xizmati, dasturiy ta’minot.",
            "text_2": "Bizning asosiy vazifamiz – mijozlarimizga sifatli xizmatlar ko‘rsatishdir, bu esa kelgusida ularning faoliyatini yaxshilashga va daromadlarini oshirishga yordam beradi. Auditorlik tashkilotlari vazifalariga muvofiq, xo‘jalik yurituvchi subyektning moliyaviy hisobotlarini tahlil qilamiz va ularning ishonchliligini tasdiqlaymiz, bu xo‘jalik faoliyatining samaradorligini real va ob’ektiv baholash asosida amalga oshiriladi. Biz xo‘jalik yurituvchi subyektlarning faoliyat samaradorligini oshirish bo‘yicha konstruktiv takliflar ishlab chiqamiz. Har bir mijozning xavf va muammolarini, shuningdek, ularning individual ehtiyojlarini aniqlash orqali, mijozning qonunchilik, faoliyat va moliyaviy hisobot tizimiga oid barcha masalalarni to‘g‘ri aks ettirishga harakat qilamiz. Bizning auditorlik tashkilotimizning obro‘si biz uchun katta ahamiyatga ega va shu sababli auditorlik sertifikatlariga ega bo‘lgan mutaxassislar tashkilotning barqaror ijobiy imidjini yaratadi va qo‘llab-quvvatlaydi."
        },
        "auditTechnology": {
            "title": "Auditorlik texnologiyasi",
            "introduction": "Hozirgi kunda zamonaviy texnologiyalar samarali auditni optimal muddatlarda amalga oshirish uchun asosiy omillardan biri hisoblanadi. Bizning auditorlik texnologiyamiz xavflarni ko'rib chiqish va tahlil qilish jarayonlarini samarali o'tkazishni ta'minlaydi va mijoz uchun qo'shimcha foyda keltiradi.",
            "section_1": "Mutaxassislarimiz tomonidan ishlab chiqilgan dasturiy ta'minot tashkilot tomonidan qo'llaniladigan yagona auditorlik metodologiyasi va hujjatlari uchun asos bo'lib xizmat qiladi. Har bir mijoz bilan bog'liq bo'lgan o'ziga xos xavf va muammolarni aniqlash orqali, bu dastur audit o'tkazish uchun mijozning individual ehtiyojlariga moslashtirilgan muhit yaratadi va mijozning qonunchilik, faoliyat va moliyaviy hisobot tizimiga oid barcha masalalarning to‘g‘ri aks ettirilishini ta’minlaydi.",
            "section_2": "Dastur auditning barcha bosqichlarini rejalashtirish, bajarish va hisobot berishni qo‘llab-quvvatlaydi va auditga oid ma’lumotlarni tahlil qilish, hujjatlashtirish, boshqarish va o‘zaro bog‘lash uchun zarur bo‘lgan vositalarni o‘z ichiga oladi."
        },
        "methodologyOfProvidedServices": {
            "title": "TA'MINLANADIGAN XIZMATLAR METODOLOGIYASI",
            "introduction": "«Javlon audit servis» audit tashkilotining audit yondoshuvi mijozning loyihasini tushunishga qaratilgan va mos rejalashtirish uchun vaqt va resurslarni sarflaydi. Bu xodimlarning ma'lumotli bo'lishini ta'minlaydi va ularga ushbu vazifani samarali bajarish imkonini beradi.",
            "section_1": "Tashkilotning yondoshuvini ajratib turadigan quyidagi xususiyatlar: xavfni baholashga asoslangan rejalashtirish; audit riskini baholash; moliyaviy hisobotni tayyorlash uchun hisob tizimi va nazoratlarni tahlil qilish; o'xshash loyihalar bo'yicha to'plangan bilimlardan foydalanish; professional fikrga e'tibor berish. Har bir auditning noyobligini tan olishimiz kerak, shuning uchun har bir loyihaga maksimal sifatga erishish uchun mos malakali xodimlar jamoasi tayinlanadi.",
            "section_2": "Bizning metodologiyamizning asosiy qismi loyihada ishlayotgan jamoa a'zolari tomonidan risklarni baholash bilan bog'liq, chunki bu risklar loyihaga ta'sir qiladi. Bizning audit metodologiyamizni tashkil etuvchi to'rt bosqich quyidagilardan iborat:",
            "points": [
                "1-bosqich - Strategik rejalashtirish: Loyihaga bog'liq maxsus muammolarni aniqlash maqsadida auditga yondoshuvimizni oldindan rejalashtirishimiz kerak. Rejalashtirish bosqichi bir qator bosqichlardan iborat bo'lib, strategiyani ko'rib chiqish va ma'lumot to'plashni o'z ichiga oladi.",
                "2-bosqich - Axborot tizimlarini ko'rib chiqish: Ushbu bosqichda audit jamoasi hisob tizimlari va nazoratlar haqida tushunchaga ega bo'ladi va ularning audit yondoshuviga ta'sirini tahlil qiladi. Ushbu tadqiqotlar natijalariga asoslanib, biz keyingi audit bosqichlari uchun batafsil ish dasturlarini ishlab chiqamiz.",
                "3-bosqich - Transaksiyalar va qoldiqlarni batafsil sinovdan o'tkazish: Loyihaning moliyaviy hisobotlarini audit qilish komponentlarga bo'linadi va har bir texnik moliyaviy hisobot komponenti bo'yicha ish hajmi professional fikrga ko'ra aniqlanadi.",
                "4-bosqich - Auditni yakunlash: Ushbu bosqichda biz korxona bo'yicha auditimiz natijalarini to'playmiz va moliyaviy hisobotlar bo'yicha fikrlar bildiramiz. Ushbu bosqich quyidagilarni o'z ichiga oladi: Hisobotlarni batafsil tahlil qilish; Moliyaviy ma'lumotlarning umumiy taqdimotini baholash; Audit jarayonida yuzaga kelgan masalalar bo'yicha rahbariyat bilan batafsil muhokama; Audit tashkilotining hamkorlari va menejerlari tomonidan bajarilgan ishni ko'rib chiqish; Moliyaviy hisobotga kiritilgan summalarning ishonchliligi va obyektiv taqdimotiga umumiy xulosa."
            ]
        },
        requiredAudit: {
            "title": "Majburiy audit",
            "introduction": "Audit — bu iqtisodiy sub'ektning moliyaviy hisobotlari va u bilan bog'liq bo'lgan moliyaviy ma'lumotlarni auditorlik tashkilotlari tomonidan ishonchliligini va moliyaviy hisobotlarning qonun hujjatlariga muvofiqligini o'rganish jarayonidir...",
            "section_1": "Quyidagi tashkilotlar majburiy yillik audit tekshiruvi ostida bo'ladi:",
            // "section_2": "Majburiy audit o'tkazish jarayonida qo'shimcha xizmatlar ko'rsatildi: soliq optimallashtirish...",
            "points": [
                "aktsiyadorlik jamiyatlari;",
                "banklar va boshqa kredit tashkilotlari;",
                "sug'urta tashkilotlari;",
                "investitsiya va boshqa fondlar, qonuniy va jismoniy shaxslardan mablag' to'playdigan va ularning ishonchli boshqaruvchilari — investitsion aktivlarni boshqaruvchi;",
                "xayriya, jamoat va boshqa fondlar, manbalari qonuniy va jismoniy shaxslardan kelib tushadigan badallar;",
                "davlatga tegishli ulushga ega bo'lgan moliya tashkilotlari va davlat unitar korxonalari;",
                "birjalar;",
                "hisoboti yilida quyidagi ikki shartdan birini bajaradigan tijorat tashkilotlari: aktivlarning balansi qiymati asosiy hisoblash miqdorining yuz ming barobardan oshishi, mahsulot (ish, xizmat) sotilishidan tushgan tushumning asosiy hisoblash miqdorining ikki yuz ming barobardan oshishi, o'rtacha yillik ishchilar soni yuzdan oshishi."
            ],
            // "section_3": "Majburiy audit tekshiruvi buyurtmachilari moliya tashkilotlaridir...",
            // "section_4": "Majburiy audit tekshiruvidan qochish ma'muriy jazolarga olib keladi..."
        },
        initiativeAudit: {
            "title": "Initsiatika Audit",
            "description_1": "Initsiatika auditi tashkilot faoliyatining samaradorligini oshirish uchun kompleks tahlil taqdim etadi.",
            "conditions": {
                "title": "Shartlar",
                "subject": "Audit tashkilot tomonidan amalga oshirilayotgan muayyan initsiatikalarga qaratiladi.",
                "terms": "Audit belgilangan vaqt ichida o'tkaziladi.",
                "responsibility": "Barcha ishtirokchilar zarur hujjatlarni taqdim etish uchun javobgardir."
            },
            "goals": {
                "title": "Maqsadlar",
                "text": "Initsiativalarning samaradorligini baholash va amaliy tavsiyalar berish."
            },
            "purposeO": {
                "title": "Initsiatika auditining maqsadi",
                "text": "Joriy initsiativalarning kuchli va zaif tomonlarini aniqlash."
            },
            "initiativeAuditCanBeConducted": [
                "Iqtisodiyotning turli sohalarida.",
                "Muayyan loyihalar yoki dasturlar uchun.",
                "Loyihani amalga oshirishning istalgan bosqichida."
            ],
            "opportunities": {
                "title": "Imkoniyatlar",
                "text": "Qaror qabul qilish jarayonini va resurslarni taqsimlashni yaxshilash."
            },
            "description_2": "Ixtiyoriy auditning buyurtmachilari qonuniy shaxslar, ularning egalari, ishtirokchilari va aktsiyadorlar bo'lishi mumkin, ular vakolatli kapitalda kamida besh foizlik ulushga ega, shuningdek, nazorat qiluvchi yoki huquqni muhofaza qilish organlari ham kiradi."
        },
        projectGrandsAudit: {
            "title": "Loyihalar va grantlar bo'yicha audit",
            "description": "O‘zbekiston Respublikasi byudjet mablag‘lari, Jahon Banki, Osiyo Taraqqiyot Banki (OTB), BMT Taraqqiyot Dasturi va boshqa nodavlat tashkilotlari tomonidan moliyalashtiriladigan loyihalar va grantlar bo‘yicha audit o‘ziga xos xususiyatlarga ega.",
            "conducting": "Audit audit standartlariga muvofiq amalga oshiriladi.",
            "auditPlan": "Loyihalar va grantlar bo‘yicha audit rejasi Jahon Banki, OTB, BMT Taraqqiyot Dasturi talablari asosida ko‘rsatmalar yoki texnik topshiriqlar asosida tuziladi va mablag‘larning manbalari va foydalanilishi haqidagi hisobotlarning, hisobga oluvchi hisoblar auditini, tatbiq etilayotgan buxgalteriya siyosatini va muhim hisob-kitoblarni baholashni o‘z ichiga oladi, shuningdek, moliyaviy hisobotlarning taqdimotini keng qamrovli baholashni o‘z ichiga oladi.",
            "documentationCheck": "Bundan tashqari, zarur qo‘llab-quvvatlovchi hujjatlarning mavjudligi tekshiriladi:",
            "documentationPoints": [
                "Sarflar bo‘yicha xarajatlarni qoplash uchun arizalarni tasdiqlovchi xarajatlar bo‘yicha hisobotlar.",
                "Moliyalashtirishga taqdim etilgan xarajatlar."
            ],
            "company": "«JAVLON AUDIT SERVIS» kompaniyasi O‘zbekiston Respublikasi/Karakalpakistonda quyidagi loyihalar (grantlar) bo‘yicha auditlarni o‘tkazgan:",
            "projects": [
                {
                    "name": "Yaponiya tomonidan Mahalliy Loyihalar uchun Grant Yordam (GAGP)"
                },
                {
                    "name": "JFPR/ADB №35447-01 ‘Karakalpakistonda hayot sifatini oshirish uchun innovatsion usullar’"
                },
                {
                    "name": "Ayollar Resurs Markazi grantlari:"
                },
                {
                    "name": "BMT aholi jamg‘armasi Grant UZB-020103-01P04"
                },
                {
                    "name": "NNO ‘DAULAT’"
                },
                {
                    "name": "ChP ‘BERAT-AGRO’"
                }
            ]
        },
        consulting: {
            title: "Konsalting",
            description: "Agar siz buxgalteriya hisobi va soliqlarni hisoblashda xatolardan qochmoqchi bo'lsangiz, agar sizda quyidagi kabi savollar tug'ilsa:",
            questions_title: "Savollar:",
            questions: [
                "Agar ustav kapitali shakllanmagan bo'lsa, soliq to'lovchining imtiyozlardan foydalanish huquqi bormu?",
                "Valyuta majburiyatlari bo'yicha hisob-kitoblar so'mda amalga oshirilsa, valyuta kursining o'zgarishi buxgalteriya hisobida qanday aks ettiriladi?",
                "Soliq to'lovchi yagona soliq to'lovi to'langanda ijara daromadini kamaytirishi mumkinmi?",
                "Hisob-faktura qanday tartibda beriladi?",
                "Agar soliq to'lovchi xarajatsiz yoki kamroq xarajat bilan qila olsa, xarajat iqtisodiy jihatdan asoslanganmi?",
                "Moliya hisobotini shakllantirishda xatolardan qochish uchun qanday qilish kerak?",
                "Qachonki QQS va nerezidentlar uchun daromad solig'ini to'lash shart emas?",
                "Agar kompaniya ishchi sayohatni tasdiqlay olmasa, u qanday qilib ishchi sayohatni tasdiqlashi mumkin va sayohat xarajatlari kompaniyaning ishlab chiqarish zaruriyatlari bilan bog'liq va ushbu xarajatlarni chegirib olish mumkinmi?",
                "Agar korxona yagona soliq to'lovi to'lovchisi bo'lsa, sotilgan konsignatsiya tovarlari qanday stavkada soliq solinadi, 5% yoki 33%?",
                "Tashkilot o'z xodimlariga berilgan bepul tovarlar, ishlar va xizmatlar qiymati ETPga kirmi?"
            ],
            offer: "Bu va boshqa ko'plab hollarda, muammolarni hal qilishni mutaxassislarga topshirib, to'g'ri qaror qabul qilish uchun vaqtni tejashingiz va, muhim ahamiyatga ega, uning amalga oshirilishi uchun pulni tejashingizni taklif qilamiz.",
            consultationAreas_title: "Maslahat sohalari:",
            consultationAreas: [
                "Buxgalteriya hisobi va moliyaviy hisobotlarni tayyorlash bo'yicha maslahat;",
                "Soliqlash va soliq deklaratsiyalarini (hisob-kitoblar, hisobotlar) tayyorlash bo'yicha maslahat;",
                "Soliqlarni optimallashtirish va ichki nazorat va buxgalteriya hisobini yaxshilash;",
                "Tashkilot tomonidan amalga oshirilayotgan biznes operatsiyalarining O'zbekiston Respublikasi qonunchiligiga muvofiqligini maslahat qilish va biznes operatsiyalarini o'tkazishda optimal sxemalarni tanlashda yordam;",
                "Hisob siyosati va boshqa ichki tartiblarni ishlab chiqish;",
                "Nazorat organlari tomonidan tekshirishlar o'tkazish vaqtida yordam va maslahatlar, kelishmovchiliklar yuzaga kelganda sizning manfaatlaringizni himoya qilish;",
                "Buxgalteriya hisobi va soliqlash sohalarida turli mavzularda seminarlar o'tkazish."
            ],
            consultationTypes_title: "Mavzular bo'yicha maslahat:",
            consultationTypes: {
                oneTime: {
                    title: "Bir martalik maslahat",
                    description: "Ushbu turdagi maslahatlarni olish uchun yozma so'rov yuborish va uni faks yoki elektron pochta orqali aniq savol bilan yuborish maqsadga muvofiq. Bizning mutaxassisimiz siz bilan telefon orqali bog'lanadi va narx va bajarish muddati haqida kelishadi. Odatda, javob yozma shaklda beriladi, lekin agar sizga og'zaki maslahat kerak bo'lsa, siz bizning ofisimizga kelishingiz mumkin, yoki biz sizga kelib, muammoni joyida hal qilishimiz mumkin."
                },
                subscription: {
                    title: "Obuna konsultatsiya xizmati",
                    description: "Ushbu turdagi maslahatlar doirasida belgilangan oylik to'lov evaziga siz kelishilgan muddat davomida yozma va og'zaki maslahatlar olishingiz mumkin. Narxlar maslahat soatlari soniga bog'liq, ammo har doim soatlik maslahatlardan arzon."
                }
            }
        },
        accounting: {
            "title": "Buxgalteriya xizmatlari",
            "description": "Bu keng tarqalgan haqiqat bo'lishi mumkin, ammo, shunga qaramay, zamonaviy sharoitlarda o'z biznesingizni samarali boshqarish uchun o'z vaqtida va ishonchli ma'lumotlar bo'lishi shart, bu sizning tashkilotingizda/biznesingizda aniq tuzilgan buxgalteriya hisobi tizimi bilan ta'minlanadi. Buxgalteriya bo'limining tashkilotning/moliyaviy holatiga ta'siri ham sir emas. Bu ta'sir shundaki, buxgalteriya:",
            "points": [
                "Tashkilotning xo'jalik operatsiyalarini amalga oshirishda O'zbekiston qonunchiligiga rioya qilinishini va ularning maqsadga muvofiqligini nazorat qilish;",
                "Tashkilotning aktivlari mavjudligi va harakati ustidan nazorat;",
                "Mehnat va moliyaviy resurslardan foydalanishni nazorat qilish;",
                "Yetkazib beruvchilar bilan to'lovlar (ko'rsatilgan xizmatlar) bo'yicha hisob-kitoblarning aniqligi va o'z vaqtida bajarilishi."
            ],
            "importance": "Shu sababli, tashkilotning buxgalteriya bo'limida yuqori malakali mutaxassislar ishlashi juda muhimdir, buxgalteriya hisobi va soliqlar sohasida bilimga ega bo'lish bilan birga, mehnat va fuqaro qonunchiligi normalariga ham ega bo'lishi kerak. Ammo, bunday xodimlar mavjud bo'lsa ham, ko'pincha tashqi va mustaqil mutaxassislarni jalb etishni talab qiladigan bir qator savollar yoki muammolar paydo bo'ladi.",
            "service_offer": "Buxgalteriya xizmatlari hozirda beriladigan xizmatlar turi nuqtai nazaridan eng foydali va talabga ega takliflardan biridir. Foyda shundaki, 'JAVLON AUDIT SERVIS' MChJ mutaxassislarining tajribasi sizning tashkilotingiz xodimlarining tajribasini to'ldiradi, bu xodimlar joriy ish bilan band bo'lishi mumkin va ayrim noaniqliklarni ko'rmasligi mumkin. Hamkorlik bu noaniqliklarni tushunishga va xodimlaringizni ularni eng kam xarajat bilan hal qilishga o'rgatishga yordam beradi.",
            "contract_benefits": "Agar siz biz bilan buxgalteriya xizmatlari ko'rsatish bo'yicha shartnoma tuzsangiz, hisobotlar to'liq, kech yoki qoidalarni buzib berilganligi haqida xavotir olmaysiz. Siz soliq va buxgalteriya qonunchiligini buzish bilan bog'liq jarimalar va to'lovlar xavfini kamaytirishingiz mumkin.",
            "quality_assurance": "'JAVLON AUDIT SERVIS' MChJ buxgalteriya xizmatlarini taqdim etishda tijorat ma'lumotlarining sifatini va maxfiyligini, soliq optimizatsiyasini, ishonchlilikni va bizning ish natijalarimizni tashqi va ichki iqtisodiy ma'lumotlardan foydalanuvchilardan himoya qilishni kafolatlaydi. Professional buxgalteriya xizmatlari sizga o'z tashkilotingizning moliyaviy holati haqida tezkor ma'lumot olish imkonini beradi va zarur bo'lsa, buxgalteriya va soliq hisobi bo'yicha malakali yordam olish imkoniyatini beradi. Buxgalteriya xizmatlari ko'rsatishda, mutaxassislarimiz sizning biznesingizni rivojlantirish uchun barcha sharoitlarni yaratishga intilishadi.",
            "service_includes": [
                "Buxgalteriya hisobini tashkil etish va yuritish;",
                "Kompleks buxgalteriya xizmatlari;",
                "Inventarizatsiyalar o'tkazish: asosiy vositalar, tovar materiallari, jo'natilgan tovarlar, kelajak xarajatlar, naqd pul, xaridorlar bilan hisob-kitoblar, yetkazib beruvchilar va boshqa qarz beruvchilar va kredit beruvchilar bilan hisob-kitoblar, bank hisobi, chet el valyuta operatsiyalari va h.k.;",
                "Soliq hisob-kitoblarini tayyorlash; byudjetdan tashqari fondlarga to'lovlar bo'yicha hisob-kitoblar; chet el shaxslarining doimiy muassasalari uchun soliq deklaratsiyalarini tayyorlash.",
                "Biznes-rejalarni ishlab chiqish;",
                "Tenderlarni o'tkazish va tashkil etish;",
                "Hujjat aylanishi va kadrlar bo'limi faoliyatini tashkil etish;",
                "Tashkilotning moliyaviy faoliyatini tahlil qilish."
            ]
        }
    },
    auditConclusions: {
        title: "Audit hisobotlari",
        description: "San'atga muvofiq. «Auditorlik faoliyati to‘g‘risida»gi Qonunning 29-moddasi Auditorlik tashkiloti o‘zining rasmiy veb-saytida yoki respublika auditorlar jamoat birlashmalarining rasmiy veb-saytlarida xo‘jalik yurituvchi subyektning shaxsini tasdiqlovchi ma’lumotlar va auditorlik xulosasini ko‘rsatgan holda o‘tkazilgan majburiy auditlar to‘g‘risidagi ma’lumotlarni joylashtirishga majburdir;"
    },
    "becameAClient": {
        "warning": "Yulduzcha (*) bilan belgilangan barcha maydonlar majburiydir.",
        "title": "Mijoz bo'lish",
        "forma": {
            "name": "Ism *",
            "surname": "Familiya *",
            "email": "Email *",
            "phone": "Telefon *",
            "company": "Kompaniya nomi *",
            "role": "Sizning lavozimingiz *",
            "typeAudit": "Audit turi",
            "typeAudit_1": "Moliyaviy audit",
            "typeAudit_2": "Soliq auditi",
            "typeAudit_3": "Tijorat auditi",
            "mainJob": "Asosiy faoliyat",
            "mainJob_1": "Savdo",
            "mainJob_2": "Ishlab chiqarish",
            "mainJob_3": "Xizmatlar",
            "status": "Tashkilot holati",
            "status_placeholder": "MMT, JJT, YP, va boshqalar.",
            "time": "Audit davri",
            "time_1": "Kvartal",
            "time_2": "Yarim yillik",
            "time_3": "Yil",
            "tax": "Soliq rejasi",
            "tax_1": "Soddalashtirilgan",
            "tax_2": "Umumiy",
            "currency": "Valyuta operatsiyalari mavjudligi",
            "currency_1": "Ha",
            "currency_2": "Yo'q",
            "method": "Audit usuli",
            "method_1": "Keng qamrovli",
            "method_2": "Tanlov",
            "employees": "Tashkilotdagi xodimlar soni",
            "accounting": "Hisobchi xodimlar soni",
            "banks": "Bank hisob raqamlari soni",
            "invoices": "Taxminiy hisob-fakturalar soni",
            "revenue": "Audit davrida olingan daromad O'zbekiston so'mida",
            "turnover": "O'zbekiston so'mida aylanma",
            "date": "dd.mm.yyyy",
            "assets": "Yil oxiridagi aktivlar summasi",
            "branches": "Filiallar soni",
            "additional": "Qo'shimcha ma'lumot",
            "submit": "Jo'natish",
            "writeCount": "Soni kiriting",
            "writePrice": "Summa kiriting",
            "toast": {
                "title": "So‘rov yuborildi",
                "description": "Biz tez orada siz bilan bog'lanamiz!"
            },
            error_toast: {
                title: "Nimadir noto'g'ri ketdi",
                description: "Ba'zi xatolik yuz berdi. Qayta urinib ko'ring!"
            }
        }
    },
    "help": {
        "topText": {
            "help": {
                "title": "Yordam",
                "training": {
                    "title": "O'qitish",
                    "content": [
                        "Firma xodimlarining kasbiy rivojlanishini rag'batlantirish maqsadida rahbariyat kasbiy tan olingan ta'lim olish istagidagi xodimlarni sertifikatlash yo'li bilan qo'llab-quvvatlaydi.",
                        "Kasbiy maqomni tasdiqlovchi dasturlar O'zbekiston Respublikasi Moliya vazirligining auditor sertifikatini olish va sertifikatlangan auditor (CPA) bo'lish dasturlarini o'z ichiga oladi. Ushbu tan olingan imtihonlarni muvaffaqiyatli topshirish shaxsiy o'sishga olib keladi va kasbiy maqomni oshiradi.",
                        "Xodimlarni kasbiy sertifikatlash orqali kasbiy tan olishga undash maqsadida Firma quyidagi yordamni ko'rsatishi mumkin:",
                        [
                            "Tan olingan tayyorgarlik kurslarining xarajatlarini to'lash. Noaniqliklarning oldini olish uchun tanlangan kurslar Firmaning rahbariyati tomonidan oldindan tasdiqlanishi kerak. Kurslarga qatnashish ish vaqtidan tashqarida (jumalarning kechki paytlari) yoki afzal ko'riladigan holatda dam olish kunlarida rejalashtirilishi lozim.",
                            "Imtihonlarda qatnashishga sarflangan vaqt ruxsatli sabab sifatida qabul qilinadi.",
                            "Firma kasbiy bilimlarni oshirish va kasbiy standartlarga rioya qilish orqali sertifikat olish imkoniyatini beradi."
                        ]
                    ]
                },
                "program": {
                    "title": "Firmaning xodimlarini 2007 yilga o'qitish dasturi",
                    "table": [
                        {
                            "number": 1,
                            "group": "Rahbariyat",
                            "topic": "Menejment, moliya va audit",
                            "planned_date": "Kurslar tashkil etilgan paytda",
                            "actual_date": "+2007 yil",
                            "responsible": ""
                        },
                        {
                            "number": 2,
                            "group": "Auditorlar",
                            "topic": "Auditorlarning malakasini oshirish kurslari",
                            "planned_date": "Kurslar tashkil etilgan paytda",
                            "actual_date": "+2007 yil",
                            "responsible": "O'zbekiston Auditorlar Palatasi"
                        },
                        {
                            "number": 3,
                            "group": "Auditorlar va auditor yordamchilari",
                            "topic": "Ichki seminarlar (audit va buxgalteriya hisobi bo'yicha)",
                            "planned_date": "Har chorakda",
                            "actual_date": "+2007 yil",
                            "responsible": ""
                        }
                    ]
                }
            }
        }
        ,
        "middleTable": {
            "title": "Audit tashkilotini tanlashda odatda ishlatiladigan texnik ko‘rsatkichlar:",
            "sections": [
                {
                    "sectionTitle": "1. Bozor barqarorligi va barqarorligi, keng qamrov / quvvat va mutaxassislar malakasi",
                    "rows": [
                        {
                            "number": "1.1.",
                            "info": "Faoliyatni amalga oshirish litsenziyasi (nusxa)",
                            "criteria": "Mustaqil audit o‘tkazish huquqini aniqlaydi",
                            "score": ""
                        },
                        {
                            "number": "1.2.",
                            "info": "Oxirgi uch yil uchun sug‘urta polislar (har bir yil uchun sug‘urta qoplamasining kamida 5%)",
                            "criteria": "Katta sug‘urta polisi mavjudligi auditorlik tashkilotining barqarorligini va sug‘urta holatida zararni qoplash imkoniyatini kafolatlaydi.",
                            "score": ""
                        }
                    ]
                },
                {
                    "sectionTitle": "2. Auditorning obro‘sini baholash uchun ma’lumotlar, auditorlik xizmatlari shartlarining sifat kafolati",
                    "rows": [
                        {
                            "number": "2.1.",
                            "info": "Filiallar mavjudligi to‘g‘risidagi ma’lumot (ijara shartnomalari, filiallar)",
                            "criteria": "Auditorlik tashkilotining keng qamrovi va quvvatini bildiradi.",
                            "score": ""
                        }
                    ]
                }
            ],
            "responsibleNote": "Ma’lumotlarni kiritish uchun mas’ul shaxs ___________________"
        },
        "bottomForm": {
            "formTitle": "Qo‘ng‘iroq buyurtmasi",
            "nameLabel": "Sizni qanday chaqiraylik?",
            "namePlaceholder": "Ismingizni kiriting",
            "phoneLabel": "Telefon raqamingiz",
            "phonePlaceholder": "Telefon raqamingizni kiriting",
            "emailLabel": "Sizning E-mail",
            "emailPlaceholder": "E-mailingizni kiriting",
            "messageLabel": "Murojaat matni",
            "messagePlaceholder": "Xabaringizni kiriting",
            "requiredMessage": "Bu maydon majburiy",
            "maxLengthMessage": "Maksimal uzunlik 400 belgidan iborat",
            "submitButton": "Yuborish",
            "toast": {
                "title": "So‘rov yuborildi",
                "description": "Biz tez orada siz bilan bog'lanamiz!"
            },
            "error_toast": {
                "title": "Nimadir noto'g'ri ketdi",
                "description": "Ba'zi xatolik yuz berdi. Qayta urinib ko'ring!"
            }
        }
    }
}